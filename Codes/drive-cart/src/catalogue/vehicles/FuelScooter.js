...
function FuelScooter({ isCart = false, props }) {
  const {
    id,
    marque,
    model,
  ...
  return (
    ...
    >
      <CardMedia
        ...
        alt={`${marque} ${model}`}
        height="200"
        image={uri ? uri : '/car-placeholder.jpg'} // Fallback image if URI is not provided
        sx={{ objectFit: 'cover' }}
      />
      <CardContent sx={{ flexGrow: 1 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          <strong>Engine Size:</strong> {engineSize} cc
          <br />
